...
enum Spin_Button_Type : uint{
    U32,
    F32,
}

struct Spin_Button_Data(T){
    T* value;
    T min;
    T max;
    T incr;
...
struct Spin_Button{
    ...
    float button_width;
    Spin_Button_Type data_type;
    union{
        Spin_Button_Data!uint  data_u32;
        Spin_Button_Data!float data_f32;
    }
}

enum Spin_Button_Text_Entry_Width = 116.0f;

private Spin_Button* add_spin_button(Gui_State* gui, Gui_ID id){
    auto font = gui.font;
    auto button_width = get_text_width(font, "+") + Button_Padding*2.0f;
    float w = Spin_Button_Text_Entry_Width + button_width*2.0f;
    float h = font.metrics.height + Button_Padding*2.0f;

    auto result = cast(Spin_Button*)add_widget(gui, id, w, h, Widget_Type.Spin_Button, Spin_Button.sizeof);
    result.button_width = button_width;
    return result;
}

void spin_button(Gui_State* gui, Gui_ID id, uint* value, uint value_min = 0, uint value_max = uint.max, uint incr = 1){
    auto btn = add_spin_button(gui, id);

    btn.data_type = Spin_Button_Type.U32;
...
void spin_button(Gui_State* gui, Gui_ID id, float* value, float value_min = -float.max, float value_max = float.max, float incr = 1){
    auto btn = add_spin_button(gui, id);

    btn.data_type = Spin_Button_Type.F32;
            ...
            if(widget){
                switch(widget.type){
                    ...
                    case Widget_Type.Spin_Button:{
                        ...
                        final switch(btn.data_type){
                            case Spin_Button_Type.U32:{
                                auto data = &btn.data_u32;
                                uint next_value;
                ...
                switch(hover_widget.type){
                    ...
                    case Widget_Type.Spin_Button:{
                        ...
                        && gui.text_input_widget != hover_widget.id){
                            ...
                            final switch(btn.data_type){
                                case Spin_Button_Type.U32:{
                                    initial_text = gen_string("{0}", (*btn.data_u32.value), allocator);
                                } break;

                                case Spin_Button_Type.F32:{
                                    initial_text = gen_string("{0}", (*btn.data_f32.value), allocator);
                                } break;
                            }
                            ...
                            end_text_input(gui);
                            // Decrement the spin button
                            final switch(btn.data_type){
                                case Spin_Button_Type.U32:{
                                    auto data = &btn.data_u32;
                                    (*data.value) = clamp((*data.value) - data.incr, data.min, data.max);
                                } break;
                ...
                final switch(widget.type){
                    ...
                    case Widget_Type.Spin_Button:{
                        ...
                        if(gui.text_input_widget == btn.id){
                            ...
                            final switch(btn.data_type){
                                case Spin_Button_Type.U32:{
                                    text_msg = gen_string("{0}", (*btn.data_u32.value), allocator);
                                } break;

                                case Spin_Button_Type.F32:{
                                    text_msg = gen_string("{0}", (*btn.data_f32.value), allocator);
                                } break;
                            }
                        }
